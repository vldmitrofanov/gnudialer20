...
xLoopEnd(const std::string & reason) { itsReason = reason; }
~xLoopEnd() {}

const std::string what() { return itsReason; }
...
public:
xConnectionError(const std::string & host) { itsHost = host; }
~xConnectionError() {}
...
public:
xFileOpenError(const std::string & filename) { itsFilename = filename; }
~xFileOpenError() {}
